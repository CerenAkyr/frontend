...
function StudentApplications(props) {
    ...
    const choices = [];
    if (status !== "CANCELED" && props.applications.semester1 !== undefined) {
        choices.push({
    ...
    if (status !== "CANCELED" && props.applications.semester2 !== undefined) {
        choices.push(
            {
    ...
    if (status !== "CANCELED" && props.applications.semester3 !== undefined) {
        choices.push(
            {
    ...
    if (status !== "CANCELED" && props.applications.semester4 !== undefined ) {
        choices.push(
            {
    ...
    if (status !== "CANCELED" && props.applications.semester5 !== undefined ) {
        choices.push(
            {
    ...
    // fetch status
    useEffect(() => {
        fetch("http://localhost:8080/application/erasmus/status/" + props.username )
            .then((response) => response.json())
