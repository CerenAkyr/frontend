...
const CoursesPage = ({loggedIn, setLoggedIn}) => {
  ...
  useEffect(() => {
    fetch("http://localhost:8080/equivalence/bystatus/ACCEPTED")
      ...
      <table>
        <thead>
          <tr>
            ...
            <th>Details</th>
            {JSON.parse(localStorage.getItem('role'))[0].authority === "ROLE_COORDINATOR" && <td>
        ...
        <tbody>
          {courses.map((course) => (
            <tr key={course.id}>
              <td>{course.externalCourseCode}</td>
              <td>{course.school}</td>
              <td>{course.bilkentCourseCode}</td>
              <td>Approved</td>
              <td>
